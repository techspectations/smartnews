		...
		for(UserTaste userTaste : userTastes){
			...
			for(Widget widget : widgetList){
				if(widget.getCategory().equalsIgnoreCase(userTaste.getCategory())){
					...
					List<NewsFlash> newsFlash = newsFlashRepository.findByCategory(userTaste.getCategory());
					List<String> flashNewsList = new ArrayList<String>();
					for(NewsFlash newsFl:newsFlash ){
						flashNewsList.add(newsFl.getNews());
					}
					widgetVo.setFlashNews(flashNewsList);
					i++;
